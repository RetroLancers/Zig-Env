...
- [ ] Implement `EnvKey` in `src/env_key.zig`
  - [ ] All struct fields
  - [ ] `init` function
  - [ ] `deinit` function
  - [ ] `clipOwnBuffer` function
  - [ ] `hasOwnBuffer` function
  - [ ] `setOwnBuffer` function
- [ ] Add tests for `EnvKey`:
  - [ ] Initialization
  - [ ] Buffer ownership
  - [ ] Clipping buffer
  - [ ] Memory cleanup
- [ ] Implement `EnvValue` in `src/env_value.zig`
  - [ ] All struct fields
  - [ ] `init` function
  - [ ] `deinit` function (with interpolations cleanup)
  - [ ] `clipOwnBuffer` function
  - [ ] `hasOwnBuffer` function
  - [ ] `setOwnBuffer` function
- [ ] Add tests for `EnvValue`:
  - [ ] Initialization
  - [ ] Interpolations ArrayList management
  - [ ] Quote state flags
  - [ ] Memory cleanup
- [ ] Implement `EnvPair` in `src/env_pair.zig`
  - [ ] Embedded key and value
  - [ ] `init` function
  - [ ] `deinit` function
- [ ] Add tests for `EnvPair`:
  - [ ] Initialization
  - [ ] Memory cleanup (cascading deinit)
- [ ] Update `src/root.zig` to export new types
